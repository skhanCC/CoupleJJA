...
def JJAResOpenEigs(Nx, Lleft, Cleft, Cin, Cjjagnd, Ljja, Cjja, Ljjdefect, Cjjdefect, Lright, Cright, Cout, kRefVec, errTol=1e-5, itNum=20, disp="on"):
    ...
    #for j in range(nCom):
        ...
        while errre > errTol or errim > errTol:
            ...
            #βLeft = (2+1j*kRef)/(2-1j*kRef) # Outgoing wavevector term
            βLeft = (2+1j*kdxleft)/(2-1j*kdxleft) # Outgoing wavevector term
            #ζLeft = Cin/( (kRef**2)*(Ctl+Cin) - (1/Ltl)*(1-βL) )
            ...
            #ζR = Cout/( (kRef**2)*(Ctl+Cout) - (1/Ltl)*(1-βR) )
            ...
            #C1D[0,0]   = Cjjagnd+Cjja+Cin-Cin*(kRef**2)*ζLeft
            ...
            #C1D[-1,-1] = Cjjagnd+Cjja+Cout-Cout*(kRef**2)*ζR
            ...
            if disp == "on":
                print('Mode: ' + str(j) + ', errre: ' + '{:.4f}'.format(np.log10(abs(errre))) + ', errim: ' + '{:.4f}'.format(np.log10(abs(errim))) + ', it. num.: ' + str(n) )
